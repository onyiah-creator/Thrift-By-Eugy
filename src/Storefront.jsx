...
// Soft tint built from a product's swatch colour — garments sit on a pale
// wash instead of the old dark gradient.
const tint = (color) => `linear-gradient(165deg, ${color}33 0%, ${color}88 100%)`;
...
function Wordmark({ onClick, compact }) {
  return (
    <button onClick={onClick} className="flex items-center gap-2.5 shrink-0" aria-label="Thrift by Eugy — home">
      <img
        src={crest}
        alt=""
        width={compact ? 30 : 38}
        ...
        style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.16)" }}
      />
      <span className="flex flex-col items-start leading-none">
        <span className="font-display italic tracking-tight" style={{ color: INK, fontSize: compact ? 17 : 21 }}>
          Thrift by Eugy
        </span>
        {!compact && (
          <span className="tracking-[0.18em] uppercase mt-1" style={{ color: GOLD_DEEP, fontSize: 8 }}>
            Curated Preloved
          </span>
        )}
      </span>
...
function Card({ p, onOpen, onSave, saved, tall }) {
  return (
    <div onClick={() => onOpen(p)} className="group cursor-pointer">
      ...
      >
        <div className="absolute inset-0" style={{ background: tint(p.color) }} />
        {p.image && (
          <img
            src={p.image}
            alt={p.name}
  ...
  return (
    <div className="min-h-screen" style={{ background: WHITE, color: INK, fontFamily: "Inter, sans-serif" }}>
      ...
      {/* Utility bar — dark strip keeps a trace of the brand's ink identity
          ...
          Ours states the scarcity fact instead of a countdown. */}
      <div className="text-center py-2 text-[11px] tracking-wide" style={{ background: INK, color: "#D8D4CC" }}>
        Every piece is one-of-one · Lagos delivery in 48hrs · 3-day returns
      </div>

      <header className="sticky top-0 z-30" style={{ background: WHITE, borderBottom: `1px solid ${HAIR}` }}>
        {/* Row 1 — logo, search, actions */}
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center gap-4">
          <button onClick={() => setMenuOpen(true)} className="md:hidden text-[19px] leading-none" style={{ color: INK }} aria-label="Open menu">
            ☰
          </button>
          ...
          <div className="flex-1 hidden md:block max-w-lg mx-auto">
            <div className="relative">
              ...
              />
              <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[13px]" style={{ color: INK_SOFT }}>⚲</span>
            </div>
          </div>

          <div className="flex items-center gap-4 ml-auto">
            <button onClick={() => setSizeGuide(true)} className="hidden lg:block text-[12px]" style={{ color: INK_SOFT }}>
              Size Guide
            </button>
            <button onClick={() => setSavedOpen(true)} className="relative text-[17px] leading-none" aria-label="Saved" style={{ color: saved.length ? "#C0392B" : INK }}>
              {saved.length ? "♥" : "♡"}
              {saved.length > 0 && (
        ...
        <nav className="hidden md:block" style={{ borderTop: `1px solid ${HAIR}` }}>
          <div className="max-w-6xl mx-auto px-4 flex items-center gap-7 h-11 text-[12px] tracking-wide uppercase">
            <button onClick={home} className="relative h-full" style={{ color: view === "home" ? INK : INK_SOFT, fontWeight: view === "home" ? 600 : 400 }}>
              Discover
            ...
            {CATEGORIES.map((c) => {
              ...
              return (
                <button key={c} onClick={() => goCat(c)} className="relative h-full whitespace-nowrap" style={{ color: on ? INK : INK_SOFT, fontWeight: on ? 600 : 400 }}>
                  {c}
                  {on && <span className="absolute left-0 right-0 bottom-0 h-[2px]" style={{ background: GOLD }} />}
                </button>
              );
            })}
            <button onClick={() => { setView("shop"); setCategory("All"); }} className="ml-auto" style={{ color: GOLD_DEEP, fontWeight: 600 }}>
              Shop All
            </button>
          </div>
        </nav>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-50 md:hidden" style={{ background: "rgba(0,0,0,0.4)" }} onClick={() => setMenuOpen(false)}>
          <div className="w-72 h-full p-5" style={{ background: WHITE }} onClick={(e) => e.stopPropagation()}>
          ...
          <section className="max-w-6xl mx-auto px-4 pt-10 pb-12 grid md:grid-cols-2 gap-10 items-center">
            ...
            <div className="grid grid-cols-2 gap-3">
              {ALL.slice(0, 4).map((p, i) => (
                <div key={p.id} className="relative overflow-hidden rounded-sm" style={{ height: i % 2 === 0 ? 175 : 215, marginTop: i % 2 === 0 ? 28 : 0, background: tint(p.color), border: `1px solid ${HAIR}` }}>
                  {p.image && (
            ...
            <div className="flex gap-7 overflow-x-auto no-sb justify-start md:justify-center">
              {CATEGORIES.map((c, i) => (
                <button key={c} onClick={() => goCat(c)} className="shrink-0 text-center group">
                  <div
                    className="relative w-[68px] h-[68px] rounded-full mb-2 transition-transform group-hover:scale-105 overflow-hidden"
                    style={{ background: `linear-gradient(150deg, ${PALETTE[i]}44 0%, ${PALETTE[i]} 130%)`, border: `1px solid ${HAIR}` }}
                  >
                    {categoryFace[c] && (
      ...
      {activeEdit && (
        <section className="max-w-6xl mx-auto px-4 py-10">
          <button onClick={() => setActiveEdit(null)} className="text-[12px] mb-5" style={{ color: INK_SOFT }}>← Back</button>
          <p className="text-[10px] uppercase tracking-[0.2em] mb-2" style={{ color: GOLD_DEEP }}>Curated by Eugy</p>
      ...
      {/* ---------------- FOOTER ---------------- */}
      {/* Ink footer — the brand's dark identity survives at the page's close */}
      ...
      {quick && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4" style={{ background: "rgba(0,0,0,0.45)" }} onClick={() => setQuick(null)}>
          <div className="w-full sm:max-w-md rounded-t-lg sm:rounded-lg overflow-hidden" style={{ background: WHITE }} onClick={(e) => e.stopPropagation()}>
            <div className="relative" style={{ height: 270, background: tint(quick.color) }}>
              {quick.image && (
            ...
            <div className="p-5">
              ...
              <p className="text-[12px] mt-1 mb-4 font-medium" style={{ color: GOLD_DEEP }}>Only one available</p>
              {lookPicks.length > 0 && (
                <div className="mb-4">
                  <p className="text-[10px] uppercase tracking-[0.15em] mb-2" style={{ color: INK_SOFT }}>
                    Complete the look
                  </p>
                  <div className="flex gap-2">
                    {lookPicks.map((p) => (
                      <button
                        key={p.sku}
                        onClick={() => open(p)}
                        className="flex-1 text-left rounded-sm overflow-hidden"
                        style={{ border: `1px solid ${HAIR}` }}
                      >
                        <div className="relative h-14" style={{ background: tint(p.color) }}>
                          {p.image && (
              ...
              {["Bag", "Details", "Payment", "Done"].map((label, i) => (
                <React.Fragment key={label}>
                  <span
                    ...
                    style={{ color: i === checkoutStep ? GOLD_DEEP : i < checkoutStep ? INK : "#B8B4AC", fontWeight: i === checkoutStep ? 600 : 400 }}
                  >
                    {label}
                  </span>
                ...
                <div className="flex-1 overflow-y-auto space-y-4">
                  ...
                  {cart.map((p) => (
                    <div key={p.id} className="flex gap-3 items-center pb-4" style={{ borderBottom: `1px solid ${HAIR}` }}>
                      <div className="relative w-16 h-20 rounded-sm overflow-hidden shrink-0" style={{ background: tint(p.color), border: `1px solid ${HAIR}` }}>
                        {p.image && (
            ...
            {checkoutStep === 1 && (
              <>
                ...
                <div className="pt-4 flex gap-2" style={{ borderTop: `1px solid ${HAIR}` }}>
                  <button onClick={() => setCheckoutStep(0)} className="px-4 rounded-full text-[13px]" style={{ border: `1px solid ${HAIR}`, color: INK_SOFT }}>Back</button>
                  <button
                    disabled={!shipping.name || !shipping.phone || !shipping.address}
                    onClick={() => setCheckoutStep(2)}
                    className="flex-1 font-medium text-[13px] py-3 rounded-full disabled:opacity-40 disabled:cursor-not-allowed"
                    style={{ background: INK, color: WHITE }}
                  >
                    Continue to Payment
                  </button>
            ...
            {checkoutStep === 2 && (
              <>
                  ...
                  {[
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: payMethod === m.id ? CREAM : "transparent",
                        color: payMethod === m.id ? INK : INK_SOFT,
                ...
                <div className="pt-4 flex gap-2">
                  <button onClick={() => setCheckoutStep(1)} className="px-4 rounded-full text-[13px]" style={{ border: `1px solid ${HAIR}`, color: INK_SOFT }}>Back</button>
                  <button
                    onClick={() => {
                      setOrderRef("TBE-" + Math.floor(100000 + Math.random() * 900000));
                      setCheckoutStep(3);
                    }}
                    className="flex-1 font-medium text-[13px] py-3 rounded-full"
                    style={{ background: INK, color: WHITE }}
                  >
                    Pay {naira(total)}
                  </button>
            ...
            {checkoutStep === 3 && (
              <div className="flex-1 flex flex-col items-center justify-center text-center">
                ...
                <button
                  ...
                  className="font-medium text-[13px] px-6 py-2.5 rounded-full"
                  style={{ background: INK, color: WHITE }}
                >
                  Continue Shopping
                </button>
